...
<v-layout>
  <v-col cols="12" sm="8" md="10">
    <div class="mb-8 mt-8" align-center>
      <h2 class="font-weight-light">List of Lecturers</h2>
      <hr>
    </div>
    <v-card>
      <v-card-title>
      ...
      <v-data-table :headers="headers" :items="lecturers" :search="search" :items-per-page="10" loading="loadTable" loading-text="Loading... Please wait" class="elevation-1">
        <template v-slot:[`item.actions`]="{ item }">
          ...
          <v-dialog v-model="deleteDialog" persistent max-width="290" :retain-focus="false">
            <template v-slot:activator="{ on, attrs }">
              <v-btn icon plain v-bind="attrs" v-on="on" color="red darken-2" @click="deleteDialog"><span class="material-icons">delete</span>
              </v-btn>
            </template>
            <v-card>
              ...
              <v-card-actions>
                ...
                <v-btn color="red darken-1" text @click="deleteLecturer(item)">
                  confirm
                </v-btn>
              </v-card-actions>
            </v-card>
